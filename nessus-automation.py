...
def main():
    ...
    if checkStatus(response1, "Login successful", "Invalid Login credentials"):
        ...
        response2 = requests.get(base_url + "/scans", headers={"X-Cookie": "token=" + token["token"]},verify=False)
        if checkStatus(response2, "Fetching scans data\n", "Unable to fetch nessus scan"):
            ...
            response3 = requests.get(base_url + "/scans/"+str(scan), headers={"X-Cookie": "token=" + token["token"]},verify=False)
            if checkStatus(response1, "Fetching data from the choosen scan", "Unable to fetch the choosen scan"):
                    ...
                    f.close()
                    finalList=[]
                    l = [dict(zip(["vulnerabilities"],[x.get("plugin_name")])) for x in scans["vulnerabilities"]]
                    for i in l:
                        i["adress_ip"]=[]
                    for j in l:
                        j["recommendation"]=[]
                    ...
                    for host in scans["hosts"]:
                        ...
                        response4 = requests.get(base_url + "/scans/"+str(scan)+"/hosts/"+str(host.get("host_id")), headers={"X-Cookie": "token=" + token["token"]},verify=False)
